...
// Admin stock adjustment — Phase 4. `delta` must be a non-zero whole
// number (positive = intake/found stock, negative = damage/loss/recount
// down) and `reason` is mandatory, per spec ("every adjustment must be
// logged" with a reason). The deeper business rules (won't go negative,
// won't dip below what's currently reserved for pending orders) are
// re-checked authoritatively inside a row lock by
// services/inventoryService.js#adjustStock — this validator only rejects
// obviously malformed input before it reaches the DB transaction.
const adjustStockValidation = [
    ...
    body('delta').notEmpty().withMessage('delta is required.')
        .isInt().withMessage('delta must be a whole number.')
        .custom((value) => {
    ...
    body('reason').trim().notEmpty().withMessage('A reason is required for every stock adjustment.')
        .isLength({ max: 500 }).withMessage('Reason must be under 500 characters.'),
    body('transaction_type').optional().isIn(['PURCHASE', 'ADJUSTMENT'])
        .withMessage('transaction_type must be PURCHASE or ADJUSTMENT.'),
    handleValidationErrors
];
